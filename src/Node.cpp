...
Engine::Node::~Node()
{
	// inform our relatives of our passing
	auto IHaveNoSon = [this]() -> void {
	if (this->parent)
		for (size_t i = 0; i < this->parent->GetChildren().size(); i++)
			if (this->parent->GetChildren().at(i) == this)
				this->parent->GetChildren().erase(this->parent->GetChildren().begin() + i);
	};

	// tell dad we're dead
	IHaveNoSon();

	// depending on the DestructionType, we have some options here
	if (Node::destuctionType == Node::DestuctionType::COLLAPSE && this->parent)

		// connect kids with their grandpa
		...
		// just leave them as orphans
		for (auto i : this->children)
			i->SetParent(nullptr);

	// now we can die in peace x.x
}
